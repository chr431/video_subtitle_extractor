...
- **批量页签**：点「批量导入…」选择文件夹，只**预览第一个视频**并列出待处理数量，
  ...
  CPU 与 GPU；实测 5 视频队列约 1.6× 加速。可在“批量输出”卡中通过
  “双引擎并行处理”开关控制（默认开启）。单个失败会继续处理并在结束时汇总
...
- **相似段合并**（默认开启，`--no-merge-similar` 关闭）：引擎在 OCR 前比较
  相邻段代表帧，若灰度平均绝对差 ≤ 3.0 **且**显著变化像素占比 ≤ 1%，则视为
  同一条字幕被噪声切成多段，合并为一段并只 OCR 一次；可显著减少高噪声/高分段
  视频的 OCR 次数，同时避免误合并“在”“不”这类单字短字幕。
