...
``video_ocr_engine.ocr.trt.TrtEngine`` 与依赖检测代码都 ``import tensorrt``；
本 shim 把它解析到同一套 thin bindings。实际推理 DLL 由引擎的
``video_ocr_engine.gpu.context`` 从 PATH 扫描本地安装的 CUDA/TensorRT 加载：
用户只需把 ``<TensorRT-xx>/bin``、CUDA ``bin`` 加入 PATH，无则引擎自动回退
OpenVINO（CPU）。
